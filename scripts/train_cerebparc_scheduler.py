    ...
    for epoch in range(meta_data["epoch"], max_epochs):
        ...
        for ix, batch_data in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                if epoch > 200:
                    # Switch over to hausdorff
                    loss_function = loss_hausdorff
                    # Reset the learning rate
                    optimizer.param_groups[0]["lr"] = (
                        math.sqrt(batch_size * num_gpus) * lr
                    )
                    if not printed_notice:
                        logger.info("Loss is now Hausdorff")
                        logger.info(f"Lr reset to: {lr}")
                        logger.info(
                            f"Optimizer lr value: {optimizer.param_groups[0]['lr']} (weighed by math.sqrt(batch_size * num_gpus))"
                        )
                        printed_notice = True
